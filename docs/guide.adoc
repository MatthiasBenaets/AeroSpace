...
. The tree is forced to be binary: any container with more than two children has its rightmost siblings
  recursively wrapped into new sub-containers until exactly two remain at each level.
...
This means that on a typical wide monitor, new windows will split horizontally first, and deeper containers
will automatically flip to vertical when their available space becomes taller than wide. The result is a
bspwm/i3-style BSP layout where splits always happen along the longest dimension.

[.lead]
*Example*

With four windows on a 1920×1080 monitor, the tree becomes:
...
The root is horizontal (wide monitor), the second level is vertical (the right half is tall), and the
third level is horizontal again (that bottom-right quadrant is wide). Splits always follow the longest axis.
